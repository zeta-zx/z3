...
export function calculateUpcomingTrack() {
    ...
    if (playerState._historyIndex < playerState._history.length - 1) {
        ...
        playerState._upcomingTrack = playerState.currentPlaylist.tracks.find(t => t.id === nextId) || null;
        if (playerState._upcomingTrack) return;
    }

    if (playerState.shuffle) {
        let candidates = playerState.currentPlaylist.tracks.filter(t => !playerState._history.includes(t.id));
        
        if (candidates.length === 0 && playerState.loop) {
            candidates = playerState.currentPlaylist.tracks.filter(t => t.id !== playerState.currentTrack?.id);
        }
        
        playerState._upcomingTrack = candidates.length > 0 ? shuffle([...candidates])[0] : null;
    } else {
        let index = playerState.currentPlaylist.tracks.findIndex(t => t.id === playerState.currentTrack?.id) + 1;
...
export function playPlaylist(playlist: Playlist, startIndex: number = 0, shuffleFlag?: boolean) {
    ...
    playerState._history = [playerState.currentTrack.id];
    playerState._historyIndex = 0;
...
export function nextTrack() {
    ...
    if (playerState._historyIndex < playerState._history.length - 1) {
        ...
        playerState.currentTrack = playerState.currentPlaylist?.tracks.find(t => t.id === nextId) || null;
    } else {
        playerState.currentTrack = playerState._upcomingTrack;
        playerState._history.push(playerState.currentTrack.id);
        playerState._historyIndex++;
    }
...
export function previousTrack() {
    ...
    if (playerState._historyIndex > 0) {
        ...
        const previousTrack = playerState.currentPlaylist.tracks.find(t => t.id === previousId);
    ...
    // if no history
    if (!playerState.shuffle) {
        let index = playerState.currentPlaylist.tracks.findIndex(track => track.id === playerState.currentTrack?.id) - 1;
        ...
        if (index !== -1) {
            ...
            playerState._history.unshift(playerState.currentTrack.id);
            playerState._historyIndex = 0;
            calculateUpcomingTrack();
        }
    }
